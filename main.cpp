...
int test_image(grg::HarrisCornerDetector harris, grg::SIFTDetector siftDetector, grg::CardRecognizer recognizer, std::string input)
{
...
int main() {
    ...
    const grg::SIFTDetector siftDetector(0, 3, 0.04, 10, 1.6);
...
//     cv::Mat imgHLS;
//     cv::cvtColor(img, imgHLS, cv::COLOR_BGR2HLS); // OpenCV folosește HLS, nu HSL exact
//
//     std::random_device rd;
//     std::mt19937 gen(rd());
//     std::uniform_int_distribution<> dH(-30, 30);  // modificări pentru Hue
//     std::uniform_int_distribution<> dL(-40, 40);  // modificări pentru Lightness
//     std::uniform_int_distribution<> dS(-40, 40);  // modificări pentru Saturation
//
...
//
//             pixel[0] = cv::saturate_cast<uchar>((h + 180) % 180); // Hue în [0, 180]
//             pixel[1] = cv::saturate_cast<uchar>(l);
//             pixel[2] = cv::saturate_cast<uchar>(s);
//         }
//     }
//
//     cv::Mat result;
...
//     std::string output_folder = "D:\\2. Area\\facultate\\card-recognision\\cards_photos\\test_hls";
//
